...
static void CMSDKAHB_GPIO_set_int_line(CMSDKAHB_GPIOState *s, int line,
                                       uint8_t level) {
...
static uint64_t CMSDKAHB_GPIO_read(void *opaque, hwaddr addr,
                                   unsigned int size) {
    ...
    case REG_INTSTATUS:
        val = s->intstatus;
        break;
    case MASKLOWBYTE_START ... MASKLOWBYTE_END:
        // unrealized
        val = s->masklowbyte;
        break;
    case MASKHIGHBYTE_START ... MASKHIGHBYTE_END:
        // unrealized
        val = s->maskhighbyte;
        break;
...
static void CMSDKAHB_GPIO_realize(DeviceState *dev, Error **errp) {
    ...
    memory_region_init_io(&s->iomem, OBJECT(s), &CMSDKAHB_GPIO_ops, s,
                          TYPE_CMSDKAHB_GPIO, 0x1000);
